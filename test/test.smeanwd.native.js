...
var smeanwd = tryRequire( resolve( __dirname, './../lib/smeanwd.native.js' ) );
var opts = {
	'skip': ( smeanwd instanceof Error )
};
...
tape( 'main export is a function', opts, function test( t ) {
	...
	t.strictEqual( typeof smeanwd, 'function', 'main export is a function' );
	t.end();
});

tape( 'the function has an arity of 3', opts, function test( t ) {
	t.strictEqual( smeanwd.length, 3, 'has expected arity' );
	t.end();
});
...
tape( 'the function throws an error if provided a first argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			smeanwd( value, new Float32Array( 10 ), 1 );
		};
	}
});
...
tape( 'the function throws an error if provided a third argument which is not a number', opts, function test( t ) {
	...
	function badValue( value ) {
		return function badValue() {
			smeanwd( 10, new Float32Array( 10 ), value );
		};
	}
});
